            ...
            <div key={group.category}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {group.items.map((service) => (
                  <Card key={service.id} className={`overflow-hidden border-none shadow-lg hover:shadow-2xl transition-all duration-300 group flex flex-col ${service.featured ? 'ring-2 ring-primary/20' : ''}`}>
                    <div className="flex-1 p-8 flex flex-col justify-between">
                      ...
                      <div className="mt-8">
                        <Button className="w-full bg-secondary hover:bg-secondary/90 text-white rounded-lg h-12 flex items-center gap-2 group-hover:translate-y-[-2px] transition-transform font-bold uppercase tracking-wider text-xs">
